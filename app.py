...
try:
    UPLOAD_FOLDER.mkdir(parents=True, exist_ok=True); RESULTS_FOLDER.mkdir(parents=True, exist_ok=True)
    app.config['UPLOAD_FOLDER'] = str(UPLOAD_FOLDER); app.config['RESULTS_FOLDER'] = str(RESULTS_FOLDER)
    log(f"Upload folder: {app.config['UPLOAD_FOLDER']}", "INFO"); log(f"Results folder: {app.config['RESULTS_FOLDER']}", "INFO")
except Exception as e: log(f"CRITICAL: Failed to create/access essential folders: {e}", "CRITICAL")

# --- Load and Store Schema Info in App Config ---
try:
    log("Loading UI schema info...", "INFO"); schema_info = parse_schema_for_ui()
    if not schema_info: log("Schema info could not be loaded/parsed.", "ERROR")
    app.config['SCHEMA_INFO_FOR_UI'] = schema_info; log("Schema info loaded into app config.", "DEBUG")
except Exception as e: log(f"CRITICAL: Failed to load schema info at startup: {e}", "CRITICAL"); app.config['SCHEMA_INFO_FOR_UI'] = {}
...
log(f"Registered API blueprints with prefix: {API_PREFIX}", "INFO")
...
# --- Restore Full Error Handlers ---
@app.errorhandler(500)
def handle_internal_error(error):
    log(f"Internal Server Error (500): {error}", "ERROR")
    log(traceback.format_exc(), "ERROR") # Log the full traceback
    # Ensure consistent JSON response for errors
    return jsonify(error="Internal Server Error", message="An unexpected error occurred on the server."), 500

@app.errorhandler(404)
def handle_not_found_error(error):
     # Log the path that wasn't found for debugging
...
def handle_method_not_allowed(error):
     ...
     # Include Allow header if Flask provides valid methods
...
if __name__ == "__main__":
    ...
    # Use environment variable or default to True for development
